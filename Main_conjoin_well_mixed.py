...
from f_gen_conjoin_well_mixed import f_gen_conjoin_well_mixed
...
潜在问题：
1.确保函数 f_baseline_disc1_disc2、f_gen_conjoin_well_mixed、f_gen_high_mat、f_gen_raplace_mat、f_gen_trans_mat、f_cal_remeet_time_two、f_cal_remeet_time_three、f_cal_remeet_time_four 和 f_get_bcratio_accumulate 已正确实现并经过测试。
2.对于大规模网络，考虑使用 scipy.sparse 优化矩阵操作。
"""
# 初始化参数
n1 = 10
n2 = 10
...
bcratio = f_get_bcratio_accumulate(madj2, 0*madj3, trans_mat, repro_val, n, retime2, retime3, 0, disc1, disc2)    %% calculate critical ratio
...
bcratio = f_get_bcratio_accumulate(madj2, madj3, trans_mat, repro_val, n, retime2, retime3, retime4, disc1, disc2)    %% calculate critical ratio
"""